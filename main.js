...
//  =====
//  * overlap/collision
//  * zoom dir changes when pressing 'x'
//
//  TODO
//  ====
//  * finish complex actions (object + subject options)
//  * drop item action
...
//  * look for memory leaks (circular references in closures)
...
defFocusLevel = 10;
  ...
  //TEXT
  textDuration	= 3000;
  answerStyle 	= 'font-size:24px;fill:yellow;';
  questionStyle = 'font-size:24px;fill:white;';
  txtStyle 	= 'font-size:24px;fill:white;';
  lookStyle	= 'font-size:24px;fill:yellow;';
...
function init()
{
  ...
  ///GLOBAL ELEMENTS
    ...
    collisionTree = new quadTreeNode( objectList.concat( scene.sceneItems ), scene.mapSizeX, scene.mapSizeY  );
  ...
  if ( defaultQuality == 'low' )
    ...
    setHighQuality();
  else if ( defaultQuality == 'very high' )
    setVeryHighQuality();
  else
    setNormalQuality();
  ...
  threadID = setTimeout( mainLoop, frameRate );
}
...
function setLowQuality()
{
  ...
  svgMain.setAttribute( 'color-rendering' , 'optimizeSpeed' );
}

function setHighQuality()
{
  svgMain.setAttribute( 'shape-rendering' , 'crispEdges' );
  svgMain.setAttribute( 'image-rendering' , 'optimizeQuality' );
  svgMain.setAttribute( 'text-rendering' , 'optimizeLegibility' );
  svgMain.setAttribute( 'color-rendering' , 'optimizeQuality' );
}

function setVeryHighQuality()
{
  svgMain.setAttribute( 'shape-rendering' , 'geometricPrecision' );
  svgMain.setAttribute( 'image-rendering' , 'optimizeQuality' );
  svgMain.setAttribute( 'text-rendering' , 'optimizeLegibility' );
  svgMain.setAttribute( 'color-rendering' , 'optimizeQuality' );
}
...
{
  svgMain.setAttribute( 'shape-rendering' , 'auto' );
  svgMain.setAttribute( 'image-rendering' , 'auto' );
  svgMain.setAttribute( 'text-rendering' , 'auto' );
  svgMain.setAttribute( 'color-rendering' , 'auto' );
}
...
function pauseGame()
{
  ...
  pausePanel.SVGNode.removeAttribute( 'display' );
}
...
function getNodeList( attribute, value, tagName ) ///get SVG nodes with that 'attribute' that match 'value', or all if no 'value'. All SVGs if no 'attribute'.
{
  ...
  else
  {
    ...
    var aux = collectionToArray( document.getElementsByTagName( 'g' ) );
    SVGlist = SVGlist.concat( aux );
  }
...
function getList( attribute, value, tagName )  ///Returns list of objects, each one of its correct object type. List contains 'attribute=value', 'attribute'=(any)', or all SVGs.
{
  ...
  for ( var i = 0 ; i < nodeList.length ; i++ )
  {
    ...
    if ( type && type != 'map' && type != 'panel' && type != 'parts' )
...
function getSVGChildById( SVGNode, id )
{
  ...
  if (  SVGNode.getAttribute( 'id' ) == id )
    ...
    for ( var child = SVGNode.firstElementChild ; child != null ; child = child ? child.nextElementSibling : null )
    {
...
function mainLoop()
{
  var suspendID = svgMain.suspendRedraw( frameRate * 1000 );
...
countDownTimer.prototype.addCountDown = function( fn, timeout, loop )
{
  loop = loop || false;
...
quadTreeNode.prototype.corresponds = function( element )	//returns true if the elemeent corresponds to this node
{
...
function icon( SVGNode, coordinates )   //most basic kind of item. Does not move. Only has a position and a shape.
{
  ...
  var d = getDimensions( this.SVGNode );
  this.width = d.width;
...
function getDimensions( node )	//Get width and height of the node, regardless of its type (svg,use,g...)
{
  var w = node.getAttribute( '_width' );
  if ( w )
    w = parseFloat( w );
  else
    w  =  node.width == undefined ? 0 : node.width.baseVal.value;

  var h = node.getAttribute( '_height' );
  if ( h )
    h = parseFloat( h );
  else
    h  =  node.height == undefined ? 0 : node.height.baseVal.value;

  return { 'width' : w , 'height' : h };
}

function setPosition( node, x, y )		//set the position on a node, regardless of its type( svg, use, g..)
{
  //ELEMENTS USING TRANSLATE TRANSFORM
  if ( node.tagName != 'svg' &&  node.tagName != 'use' )
  {
    //check for existing translate transform
    var transTransf = null;
    for( var i = 0 ;  i < node.transform.baseVal.numberOfItems ; i++ )
    {
      var transf = node.transform.baseVal.getItem( i );
      if ( transf.type == 2 )	//SVG_TRANSFORM_TRANSLATE
      {
	transTransf = transf;
	i = node.transform.baseVal.numberOfItems;
      }
    }
    //create if not found
    if ( !transTransf )
    {
      transTransf = node.ownerSVGElement.createSVGTransform();
      node.transform.baseVal.appendItem( transTransf );
    }
    //set translation
    transTransf.setTranslate( x , y );
  }

  //<use> AND <svg> USE 'x' AND 'y' ATTRIBUTES
  else
  {
...
function animElement( SVGNode, coordinates )	//icon that can move, translate and rotate. Without physics
{
  ...
  this.rotation   = undefined;          //object describing the rotatory movement
  this.scaleT = undefined;		//object describing the scaling animation (stretching)
...
animElement.prototype.move = function ()       //update for each frame, for both movement and rotation
{
  ...
  if ( this.movementList[0] || this.nextMovAction.nextMovement != undefined )
  {
	...
	if ( this.movementList[0].oriented && ( incr.x != 0 || incr.y != 0 ) )
	  ...
	  this.orientation = new vector( incr );
      }
  }
  //ELEMENT ROTATIONS
  if ( this.rotation )
...
function item( SVGNode, coordinates )   //an interactive animElement, with menu actions, and status (events).
{
  ...
  this.mirrorHView = this.SVGNode.getAttribute( 'mirrorHView' ) == 'yes';
  var viewsGroup = document.getElementById( this.id + 'views' );
  if ( viewsGroup )
    for ( var child = viewsGroup.firstElementChild ; child != null ; child = child.nextElementSibling )
      this.views.push( new view( this, child ) );

  //Get how many of the views are 'oriented'
  //The first 'view zone' is upwards
  this.viewOrientation = undefined;
  this.orientedViewCount = 0;
  for ( var i = 0 ; i < this.views.length ; i++ )
    if ( this.views[i].id.match( new RegExp( '^' + this.id + 'v[0-9]+$' , 'i' ) ) )	//special id format for oriented views
      this.orientedViewCount++;

  //Set initial view
  if ( this.orientedViewCount > 0 )
    this.setViewOrientation( this.orientation );
  else if ( this.views.length > 0 )
  {
    this.viewIndex = 0;
    this.views[0].SVGNode.removeAttribute( 'display' );
  }
  ...
  function newMenuIcon( item , action , icon, stValue, value )        //enclosure trick, to define actions with the variables of a loop
  {
    var act = function(){ action( selectedList, item ); };  //call the action on behalf of the character, over the object
    var cond = undefined;
    if ( stValue && value )
      cond = function()			//set the condition
      {
	var result = false;
	for ( var i = 0 ; i < selectedList.length && !result ; i++ )
	  result = selectedList[i].getStatusValue( stValue ) == value;
	return result;
      };
	
    return new menuIcon( item, act, icon, cond );
  }; 
  ...
  if ( actions )
  {
    ...
    {
      var match = actionList[i].match( /[a-zA-Z0-9]+=[a-zA-Z0-9]+>[a-zA-Z0-9]+/ ); 
      if ( match )			//actions based on the status of the subject 'stValue=val>action'
	//TODO actions based on the status of the object 
      {
	var parts = match[0].split( '=' );
	var stValue = parts[0];
	var value = parts[1].split( '>' )[0];
	var actStr = parts[1].split( '>' )[1];
	var act = actionDefs[ actStr ];
	var ic  = iconDefs.createInstance( actStr, icon );
	this.menuIcons.push( newMenuIcon( this, act, ic, stValue, value ) );    //do the enclosure trick to dynamically define the action with the variables of the loop
      }
      else		//simple actions, no condition needed
      {
	var act = actionDefs[ actionList[i] ];
	var ic  = iconDefs.createInstance( actionList[i], icon );
...
function view( owner, drawingGNode )
{
  this.SVGNode = drawingGNode;
  this.id = this.SVGNode.getAttribute( 'id' );
  this.animationID = undefined;
  this.owner = owner;
  this.orientation = new vector( owner.orientation );

  this.SVGNode.removeAttribute( 'style' );
  this.SVGNode.setAttribute( 'display' , 'none' );
...
item.prototype.setViewOrientation = function( dir )	//sets the view depending on orientation
{
  var numZones = ( this.mirrorHView ? this.orientedViewCount*2 : this.orientedViewCount )/2 + 1;
  if ( numZones > 1 )
  {
    var doMirror = false;		//indicates wether horizontal mirroring has to be made
    var angleShift = 2*Math.PI/numZones;
    var ang = angleCheck2( dir.angle2() + Math.PI/2 + angleShift/2 );
    var angOld = undefined;
    if ( this.viewOrientation )
      angOld = angleCheck2( this.viewOrientation.angle2() + Math.PI/2 + angleShift/2 );

    //CALCULATE THE ZONE ( '1' is upwards )
    var zone = Math.ceil( ang / angleShift );
    var oldZone = undefined;
    if( angOld )
      oldZone = Math.ceil( angOld / angleShift );

    //CHANGE THE VIEW IF NEEDED
    if ( zone != oldZone )
    {
      //CORRECT ZONE IF MIRRORING
      if ( this.mirrorHView && zone > numZones/2 + 1 )
      {
	zone = 2*( numZones/2 + 1 ) - zone;
	doMirror = true;
      }
      var animId = undefined;
      if ( this.viewIndex != undefined )
	animId = this.views[ this.viewIndex ].animationID;
      this.setView( 'v' + zone );
      this.setAnimation( animId );
      if ( this.viewIndex != undefined )
	if ( doMirror )
	  this.views[ this.viewIndex ].mirrorHoriz();
	else
	  this.views[ this.viewIndex ].SVGNode.transform.baseVal.clear();
    }

    //SAVE CURRENT ORIENTATION
    this.viewOrientation = new vector( dir );
  }
}

view.prototype.mirrorHoriz = function()
{
  if ( this.SVGNode )
  {
    var scaleTransform = this.SVGNode.ownerSVGElement.createSVGTransform();
    var translateTransform = this.SVGNode.ownerSVGElement.createSVGTransform();

    this.SVGNode.transform.baseVal.clear();
    this.SVGNode.transform.baseVal.appendItem( scaleTransform );
    this.SVGNode.transform.baseVal.appendItem( translateTransform );

    scaleTransform.setScale( -1, 1 );
    translateTransform.setTranslate( -this.owner.width/2, 0 );
  }
}

item.prototype.setView = function( newViewID )
{
  for ( var i = 0 ; i < this.views.length ; i++ )
    if ( this.views[i].id == this.id + newViewID )
    {
      this.cancelAnimations();
      if ( this.viewIndex != undefined )
	this.views[ this.viewIndex ].SVGNode.setAttribute( 'display' , 'none' );
      this.views[ i ].SVGNode.removeAttribute( 'display' );
      this.viewIndex = i;
      i = this.views.length;
    }
}
...
function animation( SVGNode )	//Collection of movements and rotations for different objects that occur at the same time. Correspond to <animation> tag
{
  ...
  for ( var child = SVGNode.firstElementChild ; child != null ; child = child.nextElementSibling )
  {
	...
	else if ( child.attributes[i].name == 'movement' )	//add movement components
	{
	...
	else if ( child.attributes[i].name == 'scale' )	//add scaling components
	  obj.scaleT = new scaling( obj.SVGNode, params[0], params[1], params[2] || obj.width, params[3] || obj.length )
      }
  }
}    

function scaling( node, vx, vy, width, length )	//object that represents the scaling transformation on a node
{
  this.sx = 0;		//scale factor 
  this.sy = 0;
  this.vx = parseFloat( vx || 0 )/100;	//scale change velocity ( percentage )
  this.vy = parseFloat( vy || 0 )/100;
  this.length = parseFloat( length || 0 );		//to fix offset
  this.width = parseFloat( width || 0 );

  this.scaleTransform = node.ownerSVGElement.createSVGTransform();
  this.translateTransform = node.ownerSVGElement.createSVGTransform();

  node.transform.baseVal.appendItem( this.scaleTransform );
  node.transform.baseVal.appendItem( this.translateTransform );
}

scaling.prototype.advance = function()
{
  this.sx += this.vx;
  this.sy += this.vy;
  this.setScaling();
...
scaling.prototype.setScaling = function( sx, sy, width, length )
{
  ...
  this.scaleTransform.setScale( 1 + this.sx , 1 + this.sy );
  this.translateTransform.setTranslate( -this.width * this.sx, -this.length * this.sy );
}

animElement.prototype.setScaling = function( sx, sy, width, length )
{
  if ( !this.scaleT )
    this.scaleT = new scaling( this.SVGNode, sx, sy, width, length );
  this.scaleT.setScaling( sx, sy, width, length );
}

function sequence( loop, bounce )	//generic sequence that changes every certain miliseconds (thought to be inherited)
{
  this.seq = [];
  this.currentIndex = 0;
...
sequence.prototype.goTo = function( index )	//the animation will stop or start over if out of bounds.
{
  ...
  if ( this.seq.length > 0 )
    mainTimer.resetCount( this.timer, this.seq[ this.currentIndex ].duration );
}
...
sequence.prototype.play = function( intervalms, bounce )	
{
  var duration = this.seq.length > 0 ? this.seq[ this.currentIndex ].duration : undefined;
  this.bounce = bounce != undefined ? bounce : this.bounce;
  intervalms = intervalms || duration;
  if( intervalms )
  {
    var that = this;
    this.timer = mainTimer.addCountDown( function(){ that.nextOrPrev() }, intervalms, true );	//'true' means loop
  }
}

sequence.prototype.nextOrPrev = function()
{
  if ( this.bounce )		//play to the end, then backwards to beginning
  {
    if ( this.currentIndex >= this.seq.length - 1 && this.backwards == false )
    {
      this.backwards = true;
      return;
    }
    else if ( this.currentIndex < 1 && this.backwards == true )
    {
      this.backwards = false;
      return;
    }
...
frameSequence.prototype = new sequence();    //new objects created with 'new' and this function will have this obj as their prototype to look up to
frameSequence.prototype.constructor = frameSequence;

function frameSequence( loop, bounce )        //sequence of frames that are displayed and hidden alternatively 
{
  sequence.call( this, loop, bounce ); 
}

frameSequence.prototype.goTo = function( index )	//the sequence will be destroyed if out of bounds.
{
  if ( index >= 0 && index < this.seq.length )
  {
    hideElement( this.seq[ this.currentIndex ].SVGNode );
    sequence.prototype.goTo.call( this, index );
    this.seq[ index ].SVGNode.removeAttribute( 'display' );
  }
}

spriteSequence.prototype = new frameSequence();    //new objects created with 'new' and this function will have this obj as their prototype to look up to
spriteSequence.prototype.constructor = spriteSequence;

function spriteSequence( SVGNode, loop, bounce )
{
  for ( var child = SVGNode.firstElementChild ; child != null ; child = child.nextElementSibling )
    this.seq.push( { 'SVGNode':child } );
}
...
function animationSequence( SVGNode, removeAtEnd, bounce )	//Sequence of timed animations
{
  sequence.call( this, removeAtEnd, bounce );
  this.id = SVGNode.getAttribute( 'id' );
  this.initialStance = undefined;
  this.bounce = SVGNode.getAttribute( 'circular' ) == 'yes';
  this.loop = true;
...
animationSequence.prototype.nextOrPrev = function()	//reverses all movement when changing backwards
{
  var oldDir = this.backwards;
  sequence.prototype.nextOrPrev.call( this );
  if ( oldDir != this.backwards )
    for ( var i = 0 ; i < this.seq.length ; i++ )
      for ( var j = 0 ; j < this.seq[i].objs.length ; j++ )
      {
	if ( this.seq[i].objs[j].rotation )
	  this.seq[i].objs[j].rotation.angVelocity = -this.seq[i].objs[j].rotation.angVelocity;
	if ( this.seq[i].objs[j].scaleT )
	{
	  var count = Math.ceil( this.seq[i].duration/frameRate );
	  this.seq[i].objs[j].scaleT.vy = -this.seq[i].objs[j].scaleT.vy/( 1 + count * this.seq[i].objs[j].scaleT.vy );
	  this.seq[i].objs[j].scaleT.vx = -this.seq[i].objs[j].scaleT.vx/( 1 + count * this.seq[i].objs[j].scaleT.vx );
	  if ( !this.backwards )
	  {
	    this.seq[i].objs[j].scaleT.sy = 0;
	    this.seq[i].objs[j].scaleT.sx = 0;
	  }
	}
	for ( var k = 0 ; k < this.seq[i].objs[j].movementList.length ; k++ )
	  this.seq[i].objs[j].movementList[k].direction = this.seq[i].objs[j].movementList[k].direction.multiply( -1 );
      }
}

animationSequence.prototype.stop = function()
{
  sequence.prototype.stop.call( this );

  //RESTORE THE ORIGINAL DIRECTIONS IF GOING BACKWARDS
  if ( this.backwards )
    for ( var i = 0 ; i < this.seq.length ; i++ )
      for ( var j = 0 ; j < this.seq[i].objs.length ; j++ )
      {
...
function stanceItem( SVGNode )
{
  ...
  this.angle = parseFloat( SVGNode.getAttribute( 'angle' ) || 0 );
  ...
  this.scale = new vector( SVGNode.getAttribute( 'sx' ) || 0 , SVGNode.getAttribute( 'sy' ) || 0 );
  this.scaleCenter = new vector( SVGNode.getAttribute( 'scx' ) || 0 , SVGNode.getAttribute( 'scy' ) || 0 );
}
...
item.prototype.setAnimation = function( newAnimID )
{
  if ( this.viewIndex != undefined )
  {
    var v = this.views[ this.viewIndex ];
    v.animationID = newAnimID;
    this.cancelAnimations();
    for ( var i = 0 ; i < v.animations.length ; i++ )
      if ( v.animations[i].id == v.id + newAnimID )
      {
	v.animIndex = i;
	v.animations[ v.animIndex ].play();	//set the timer for changing the sequence
	this.setStance( v.animations[ v.animIndex ].initialStance );
	i = v.animations.length;
      }
  }
}
...
item.prototype.cancelAnimations = function()	//stop any animation and bring the stance back to original
{
  ...
  {
    var v = this.views[ this.viewIndex ];
    if ( v.animIndex != undefined )
      v.animations[ v.animIndex ].stop();	//remove timer for animation sequence
    this.removeAnimationStance();		//reset transformations due to current animation
    v.animIndex = undefined;			//unset animation
  }
}
...
item.prototype.removeAnimationStance = function()	//reset transformations due to current animation
{
  if ( this.viewIndex != undefined )
  {
    ...
    if ( v.animIndex != undefined )
    {
      var seq = v.animations[ v.animIndex ];
      if ( seq.initialStance )
	for ( var j = 0 ; j < seq.initialStance.stanceItems.length ; j++ )
	{
	  ...
	  stItem.obj.setScaling( 0 , 0 );
	}

      for ( var j = 0 ; j < seq.seq.length ; j++ )
      {
	var anim = seq.seq[j];
...
item.prototype.setStance = function( st )
{
  ...
  if ( st )
    for ( var i = 0 ; i < st.stanceItems.length ; i++ )
    {
      ...
      st.stanceItems[i].obj.setScaling( st.stanceItems[i].scale.x, st.stanceItems[i].scale.y,
					 st.stanceItems[i].scaleCenter.x, st.stanceItems[i].scaleCenter.y );
    }
}
...
item.prototype.animate = function()
{
  ...
  {
    var v = this.views[ this.viewIndex ];
    if ( v.animations.length > 0 && v.animIndex != undefined )
    {
      var seq = v.animations[ v.animIndex ];
      seq.seq[ seq.currentIndex ].animate();
    }
  }
}
...
function menuIcon ( obj, action, icon, condition )	//represents a skill over an object
{
...
item.prototype.setCMenu = function( menuIcons, radius, itemSize ) //layout the menu icons in a circle
{
  ...
  //Remove menuIcons that don't meet the condition
  for ( var i = 0 ; i < menuIcons.length ; i++ )
    if( menuIcons[i].condition == undefined || menuIcons[i].condition() )
      activeIcons.push( menuIcons[i] );

  //Add active character actions TODO
  //for ( var i = 0 ; i < selectedList.length ; i++ )
    //for ( var j = 0 ; j < selectedList[i].actions.length ; j++ )
      //if( selectedList[i].actions[j].condition == undefined || selectedList[i].actions[j].condition() )
	//activeIcons.push( selectedList[i].actions[j] );

  //Empty menuGroup
  for ( var child = this.menuGroup.firstElementChild ; child != null ; child = aux )
  {
    var aux = child.nextElementSibling;
  ...
  for ( var i = 0 ; i < activeIcons.length ; i++ )
  {
    ...
    itemGroup.appendChild( activeIcons[i].icon.SVGNode );

    //Scale each icon to fit the tile
    var itemWidth  = parseFloat( activeIcons[i].icon.SVGNode.getAttribute( 'width'  ) );
    var itemHeight = parseFloat( activeIcons[i].icon.SVGNode.getAttribute( 'height' ) );
    var scaleFactor = itemSize / Math.max( itemWidth, itemHeight );
    itemGroup.setAttribute( 'transform', 'scale( ' + scaleFactor + ',' + scaleFactor + ')' );

    setPosition( activeIcons[i].icon.SVGNode , 
      ( radius * Math.cos( i * delta - Math.PI/2 ) - itemWidth/2  )/scaleFactor ,
      ( radius * Math.sin( i * delta - Math.PI/2 ) - itemHeight/2 )/scaleFactor ); 
...
item.prototype.showCMenu = function()
{
  ...
  this.setSCircle();
  this.circNode.removeAttribute( 'display' );
}
...
character.prototype.drawInventory = function( pos, width, height, numItemsRow, leftToRight ) //Draw the inventory on the panel
{
  ...
  {
    //Scale each icon to fit the tile
    var d = getDimensions( child.firstElementChild );
    var scaleFactor = itemSize / Math.max( d.width, d.height );
    ...
    //Place the icon
    setPosition( child.firstElementChild, place/scaleFactor, ( pos.y + row * itemSize )/scaleFactor );
...
{
  var wheelData = event.detail ? -event.detail : event.wheelDelta / 40;	//normalize FF and Chrome the FF way
...
textSequence.prototype = new frameSequence();    //new objects created with 'new' and this function will have this obj as their prototype to look up to
...
function textSequence( str, style, pos, lines, chPerLine, width, removeAtEnd )        //sequence of text objects. Separated each number of lines and/or by the separator '||'
{
  ...
  for ( var j = 0 ; j < lineGroups.length ; j++ )
  {
    ...
    txtPart.lines.shift();				//remove first empty line
    ...
    //Add to the sequence
    this.seq.push( txtPart );
    hideElement( txtPart.SVGNode );
  }
  this.seq[0].SVGNode.removeAttribute( 'display' );
}
...
textSequence.prototype.goTo = function( index )	//the sequence will be destroyed if out of bounds.
{
  if ( index < 0 || index >= this.seq.length )
  {
    ...
    frameSequence.prototype.goTo.call( this, index );
}
...
conversationTreeNode.prototype.show = function ( pos )	//shows the conversation menu, generating answer nodes
{
  ...
  pos = pos.minus( 0 , menuRowDistance * ( this.answers.length ) / 2 );	//center the text TODO
  this.questionTXT = new textSequence( this.question, this.style, pos );
  this.questionTXT.removeAtEnd = false;
  this.questionTXT.play( textDuration );

  //DRAW ANSWER NODES
  pos = pos.add( 0 , menuRowDistance/2 );
