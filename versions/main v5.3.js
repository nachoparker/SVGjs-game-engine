...
//  TODO
//  ====
//  * mass,speed as statusValues, collisions as events..
...
function gotest( event )//TODO this is only a test REMOVE
{
  ...
  event.args[0].statusValues[0].setValue( -50 );
}
function gotest1( event )//TODO this is only a test REMOVE
{
  event.args[0].addGoToCBehaviour( new vector( 0, 0), true ); //'true' means its an order
}
...
function eventListener( type, action, conditionStr, oneUse )
{
  ...
  if ( match )
  {
    ...
    this.condition = function( event ){ return event.conditionStr == conditionStr };
}
...
{
  objList = objList || scene.itemList;
  for ( var i = 0 ; i < objList.length ; i++ )
...
function item( SVGNode, coordinates )   //an interactive element, without physics
{
  ...
  if ( statusValuesGroup )
    for ( var child = statusValuesGroup.firstElementChild ; child != null ; child = child.nextElementSibling )
      this.addStatusItem( child.getAttribute( 'name' ),
			  child.getAttribute( 'value' ),
			  child.getAttribute( 'type' ) );
}

function statusValue( name, value, type, rootObj, evDispatcher ) //a value that is part of the status of an element
...
{
  if ( !this.validationFunc || ( this.validationFunc && this.validationFunc() ) )
  {
    this.value = value;
...
item.prototype.addStatusItem = function( name, value, type )
{
  this.statusValues.push( new statusValue( name, value, type, this, this.evDispatcher ) );
}
