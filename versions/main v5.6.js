...
function animElement( SVGNode, coordinates )	//icon that can move, translate and rotate. Without physics
{
  ...
  if ( this.SVGNode.tagName != 'use' )	//movements on use elements use 'x' and 'y' attributes, whereas the rest don't have them and use translate transformation
  {
...
animElement.prototype.setRotation = function( angle, center )
{
  if ( angle != undefined )
    this.angle = angle;
  this.rotation.center = center ? center : this.rotation.center;
  this.rotation.center = this.rotation.center ? this.rotation.center : this.pos;

   if ( this.rotateTransform )
    this.rotateTransform.setRotate( this.angle*180/Math.PI, this.rotation.center.x, this.rotation.center.y );
  else
    this.SVGNode.setAttribute( 'transform', 'rotate( ' + this.angle*180/Math.PI + ',' + this.rotation.center.x + ',' + this.rotation.center.y + ')' );
}
...
{
  intervalms = intervalms || this.sequence[ this.currentIndex ].duration;
  var that = this;
...
function animationSequence( SVGNode, removeAtEnd )	//Sequence of timed animations
{
  ...
  this.id = SVGNode.getAttribute( 'id' );
  this.circular = SVGNode.getAttribute( 'circular' ) == 'yes';	//Indicates that at the end, the inverse animation will be performed, so that at the end we 
								//are at the initial position again.
  ...
  if ( this.circular )	//go from the last movement to the first one, inverting them and attaching them to the sequence
	...
	if ( anim.objs[i].rotation )
	  anim.objs[i].rotation.angVelocity = -1*anim.objs[i].rotation.angVelocity;	//we invert the movement
...
item.prototype.setAnimation = function( newAnimID )
{
  for ( var i = 0 ; i < this.animations.length ; i++ )
	...
	this.animations[ this.animIndex ].stop();
      this.animIndex = i;
