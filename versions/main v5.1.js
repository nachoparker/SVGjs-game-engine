...
//  ====
//  * animations, barritas,
//  * actions: talk! (go before looking, taking...)
//  * triggers
//  * animated sequences
...
function init()
{
  ...
  elementDefs = [];//TODO load from external document
  elementDefs[ 'map'  ] = map;
  ...
  //txtPanel = new text();
  //txtPanel.addMouseFunction();	//clean text on click
...
function lookItem( chList, it, duration )     //print the text of the 'description' attribute on the SVG node of the object
{
  ...
  var txt = new textSequence( it.SVGNode.getAttribute( 'description' ) || 'No hay nada interesante que ver' ,
      							'fill:yellow;font-size:24px;stroke:black;stroke-width:0.2px',
							undefined, scene.toScreen( it.pos ) );
  txt.play( duration );
}
...
//TODOd put these functions somewhere else
...
function textSequence( str, style, lines, pos, chPerLine, width )        //sequence of text objects. Separated each number of lines and/or by the separator '||'
{
  lines = lines || 5;
  this.txtSeq = [];    
  this.currentIndex = 0;
...
textSequence.prototype.remove = function()
{
...
//{}
//
//bar.prototype.setValue = function(){}           //TODO set events at certain values??
//
