...
function init()
{
  ...
  itemList = allList.find( item/*, 'item' */);
  beingList = allList.find( being/*, 'item' */);
  characterList = allList.find( character /*, 'character'*/ );
  selectedList = [];
...
function getList( attribute, value, tagName )  ///Returns list of objects, each one of its correct object type. List contains 'attribute=value', 'attribute'=(any)', or all SVGs.
{
  ...
  for ( var i = 0 ; i < nodeList.length ; i++ )
	...
	list.push( new object( nodeList[i].getAttribute( 'id' ) ) );
	...
	list.push( new item( nodeList[i].getAttribute( 'id' ) ) );
      else if ( nodeList[i].getAttribute( 'element' ) == 'being' )
	list.push( new being( nodeList[i].getAttribute( 'id' ) ) );
    }
...
sceneItem.prototype.getShape = getShape;//TODO as subclass of item??

item.prototype.collisionTrigger = function( obj, collisionInfo )	//executes the reaction to a collision
{
//   this.stop();
...
{
  object.call( this, id, coordinates );
  if ( !id ) return;
...
{
  being.call( this, id, coordinates );
  if ( !id ) return;
...
//     objectList[0].drawPosition( new vector(0,0) ) ;
...
function map( id, viewBoxCoords, viewBoxSizeX, viewBoxSizeY ) ///coordinates are CENTER of the view box
{
  ...
  this.mapSizeX  = this.SVGNode.width.baseVal.value;
  this.mapSizeY = this.SVGNode.height.baseVal.value;
...
//     this.width  = this.SVGNode.width.baseVal.value;
//   this.height = this.SVGNode.height.baseVal.value;TODO
