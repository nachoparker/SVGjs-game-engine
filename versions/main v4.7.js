...
//	TODO
//	====
//	* behaviours: group behaviours (behaviour manager) : in line, gather, move in mass to a point, keep formations + efficiency in avoid
//	* square modulus
...
element.prototype.addMovement = function( newMovement, newSpeed )			//add movement next frame
{
...
element.prototype.replaceMovement = function( newMovement, newSpeed )		//replace movement next frame
{
  if ( newMovement && newMovement instanceof movement )
  {
...
element.prototype.move = function ()  			//update for each frame, for both movement and rotation
{
  ...
  if ( this.rotation )
  {
    ...
    else
    {
      ...
      if ( this.rotation.targetAngle != undefined && Math.abs( this.rotation.targetAngle - this.angle ) < Math.abs( this.rotation.angVelocity ) )//TODO, limit of range
      {
...
element.prototype.updateBehaviours = function( bhList )		//checks if any trigger needs to be executed and changes the state accordingly
{
...
element.prototype.makeBasicInfBehaviour = function( move, code, priority, time )	//creates a one-state behaviour that repeats everytime a movement is finished
{
...
element.prototype.addBasicInfBehaviour = function( move, code, priority, time )	//creates a one-state behaviour that repeats everytime a movement is finished
{
...
element.prototype.addComplexBehaviour = function( cbh, cbhList )
{
...
element.prototype.removeComplexBehaviour = function( cbh, cbhList )
{
  cbhList = cbhList || this.complexBehaviours;
...
element.prototype.updateComplexBehaviours = function( cbhList )	//removes c. behaviours components that have expired ( from any list, or from all behaviours )
{
  cbhList = cbhList || this.complexBehaviours;
  for ( var i = 0 ; i < cbhList.length ; i++ )
...
element.prototype.keyboardControlOrders = function()
{
  this.orders = [];
  var that = this;
...
element.prototype.addGoToCBehaviour = function( destPos, order )		//the element goes to the location, while avoiding obstacles
{
