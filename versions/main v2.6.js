...
movement.prototype.randomize = function()	//TODO review random functions
{
  if ( this.RSpeed )
  {
    this.RSpeed.count++;
...
//     this.keyHandler = undefined;
    ...
    if ( event.keyCode == 84 ) ///'t' FORWARD
    {
	this.replaceMovement( new movement( this.orientation ) );
	this.setState ( 'moving' );
    }
    if ( event.keyCode == 71 ) ///'g' BACKWARD
    {
    ...
    {
      if ( !this.movementList[0] )		///new simple movement upwards
      {
	this.orientation = new vector( 0, -1 );
	this.replaceMovement( new movement( this.orientation ) );
	this.setState ( 'moving' );
      }
      else					///influence the current movement by adding an upward vector
      {
	this.orientation = this.movementList[0].direction.add( 0, -1 ).direction();
	this.movementList[0].direction = this.orientation;
      }
    }
	...
	this.setState ( 'moving' );
      ...
      {
	this.orientation = this.movementList[0].direction.add( 1, 0 ).direction();
	this.movementList[0].direction = this.orientation;
      }
    }
...
{
    if ( event.keyCode == 87 || event.keyCode == 83 || event.keyCode == 68 || event.keyCode == 65 || event.keyCode == 84 || event.keyCode == 71 )
...
function mouse( event )	//click in the map causes selected elements to move there immediately, later(shift). Curvedly with ctrl
{
  for ( var i = 0 ; i < elementList.length ; i++ )
    if ( elementList[i].selected )
      elementList[i].moveTo( new vector( event.clientX, event.clientY ), event.shiftKey, event.ctrlKey );
}
...
map.prototype.zoom = function ( newPoint, newSizeX, newSizeY )	///set camera to this position if parameters, perform zoomState otherwise.
{
  if( newPoint && newSizeX && newSizeY )
    this.setViewBox( /*new vector( newPoint.x-newSizeX/2 , newPoint.y-newSizeY/2 )*/ newPoint , newSizeX, newSizeY );
...
map.prototype.follow = function ( object, centered )			///Pan camera as object moves 
{
/*    if ( object && object.movementList[0] )
    {
