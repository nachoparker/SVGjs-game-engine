...
//	* behaviours:
//			-orders
//			-review efficiency(quadtrees..)
//			-group behaviours (behaviour manager) : in line, gather, move in mass to a point, remove behaviours
//	* review movementlist
//	* square modulus
//	* new movement: accelerate to final speed
...
function movement ( owner, initialDir, speed, friction, oriented, pauseBehaviours )
{
  ...
  this.pauseBehaviours = ( pauseBehaviours == undefined ) ? true : pauseBehaviours ;		//marks the movement as belonging to a behaviour
...
detourMovement.prototype.constructor = movement;
...
element.prototype.distanceTo = function( checkPoint )			///returns distance from the center to the point
{
  return this.pos.minus( checkPoint ).modulus();
}

element.prototype.isCloserTo = function( checkPoint, radius )		///check if the element hits a point/zone
{
  var result = false;
...
//   this.stop();
  ...
  if ( obj instanceof element )
    this.addAvoidBehaviour( obj, 25 + Math.max( this.width, this.height ) + Math.max( obj.width, obj.height ) );
}
...
element.prototype.updateBehaviours = function()		//checks if any trigger needs to be executed and changes the state accordingly
{
  var result = false;
  if ( this.behaviours.length > 0 )
    for ( var j = 0 ; j < this.behaviours.length ; j++ )
    {
      if ( this.behaviours[j].state.timer <= 0 )
	this.behaviours.splice( j, 1 );	//remove the behaviour if it expired
      else
	...
	this.behaviours[j].state.timer--;
	if ( this.behaviours[j].state.update )
	  this.behaviours[j].state.update();
	if ( this.behaviours[j].state.movement )
	  this.behaviours[j].state.movement.weight = this.behaviours[j].priority + this.behaviours[j].state.priority;
	...
	for ( var i = 0 ; i < this.behaviours[j].state.triggers.length ; i++ )
	  if ( maxPriority < this.behaviours[j].state.triggers[i].priority
	    && this.behaviours[j].state.triggers[i].fire != undefined
	    && this.behaviours[j].state.triggers[i].fire() == true )
	  {
	...
	if ( aTrigger && aTrigger.nextState )		//undefined 'nextState' means the current state is not abandoned
	{
	  ...
	  this.behaviours[j].state.timer = this.behaviours[j].state.inicTime;
	}
      }
    }
  return result;
...
element.prototype.doBehaviours = function()		//executes the new movement if the state changed
{
    ...
    || !this.movementList[0] )	//behaviours should be paused while there are other movements (orders) going on (until they have stopped)
  {
    ...
    this.updateComplexBehaviours();
    var move = false;
    for ( var i = 0 ; i < this.behaviours.length ; i++ )
      if ( this.behaviours[i].state.movement )
      {
	this.behaviours[i].state.movement.components = [];	//only permit simple movements for each behaviour
	if ( !move )
...
element.prototype.addBehaviour = function( bh )			//adds a behaviour only if it did'nt exist already
{
  var found = false;
  for ( var i = 0 ; i < this.behaviours.length ; i++ )
  {
...
element.prototype.removeBehaviour = function( bh )
{
  for ( var i = 0 ; i < this.behaviours.length ; i++ )
...
element.prototype.addBasicInfBehaviour = function( move, priority, time, code )	//creates a one-state behaviour that repeats everytime a movement is finished
{
  var state = new BState( move, undefined, undefined, time );	//create a state with a corresponding movement
  var that = this;
  this.addBehaviour( new behaviour( state, code, priority ) );
}

element.prototype.increaseBasicInfBehaviour = function( move, code, time, incr )	//increases priority or creates behaviour
{
  if ( !this.increaseBehaviourPriority( code, incr ) )
    this.addBasicInfBehaviour( move, undefined, time, code );
}

element.prototype.addBasicBehaviour = function( move, time, priority, code )	//creates a one-state behaviour that is removed on the idle state (after a 'time')
{
  ...
  var that = this;
  state0.addTrigger( function(){ return that.state == 0 } , state1 );		//add a condition to re-initiate the state/movement: if the object is idle
  this.addBehaviour( new behaviour( state0, code, priority ) );
  this.setState( 1 );
}

element.prototype.increaseBasicBehaviour = function( move, code, time, incr )	//increases priority or creates behaviour
{
  if ( !this.increaseBehaviourPriority( code, incr ) )
    this.addBasicBehaviour( move, time, undefined, code );
}

element.prototype.addChaseBehaviour = function( target, distance, code, basePriority )//chases the target if it gets too far, and flees from it if it gets too close
{
...
element.prototype.addAvoidBehaviour = function( target, distance, code, maxPriority, basePriority )	//flees from the target if it gets too close
{
...
element.prototype.makeDetourBehaviour = function( obj, distance, target, code, maxPriority, basePriority )	//moves away from and around an obstacle
{
  code = code || "DE-" + obj.id;
  ...
  state1.addTrigger( function() {
			...
			return true;
		      ...
		      return false;
			; }
			, state0 );	//if far enought, idle
  return new behaviour( state0, code, basePriority );
}

element.prototype.addDetourBehaviour = function( obj, distance, target, code, maxPriority, basePriority )	//moves away from and around an obstacle
{
  this.addBehaviour( this.makeDetourBehaviour( obj, distance, target, code, maxPriority, basePriority ) );
}

element.prototype.addDetourAllBehaviour = function( dist, target, code, maxPriority, basePriority )	//moves away from and around an obstacle
{
...
element.prototype.addAvoidObstacleBehaviour = function( obj, distance, code, maxPriority, basePriority )	//flees from an obstacle
{
  code = code || "AO-" + obj.id;
  basePriority = basePriority || 0;
  maxPriority = maxPriority || 4;
  ...
  state1.addTrigger( function() { return obj.distanceFromFaceTo( that.pos ) > distance; }, state0 );	//if far enought, idle
  this.addBehaviour( new behaviour( state0, code, basePriority ) );
}

element.prototype.addGoToBehaviour = function( destPos, code, basePriority )	//the behaviour repeatedly goes to the destination and then is removed
{
  this.addBehaviour( this.makeGoToBehaviour( destPos, code, basePriority ) );
  this.setState( 1 );
}

element.prototype.makeGoToBehaviour = function( destPos, code, maxPriority, basePriority )	//the behaviour repeatedly goes to the destination and then is removed
{
  ...
  var state0 = new BState( new rectMovement( that, destPos ) ,			//create a state with a corresponding movement
			    function(){
			      // Priority goes linearly from '0' when far, to 'maxPriority' when too close
			      var dist= that.distanceTo( destPos );
			      if ( dist < 50 )
				this.priority = maxPriority*( 1 - dist/50 );
			      this.priority = this.priority < 0 ? 0 : this.priority; }
			  );

  var state1 = new BState( undefined, undefined, undefined, 0 );		//idle state, duration '0'

  state0.addTrigger( function() { return that.isCloserTo( destPos, 1 ); }, state1 );	//on arrival, stop
  return new behaviour( state0, code, basePriority, true );
}

element.prototype.addKeepPositionBehaviour = function( code, basePriority )	//always tries to stay on that position
{
  code = code || 'KP';
  basePriority = basePriority || 1;
...
// {
//   this.objList = objList || elementList;
//   this.refresh = refresh;
//   init.call( this );
}

function complexBehaviour ( bhList, finishCheck )
{
  this.bhList = bhList || [];
...
element.prototype.addComplexBehaviour = function( cbh )
{
  this.complexBehaviours.push( cbh );
  for ( var i = 0 ; i < cbh.bhList.length ; i++ )
    this.addBehaviour( cbh.bhList[i] );
}

element.prototype.removeComplexBehaviour = function( cbh )
{
  for ( var i = 0 ; i < this.complexBehaviours.length ; i++ )
    if ( this.complexBehaviours[i] === cbh )
      this.complexBehaviours.splice( i, 1 );
  for ( var i = 0 ; i < cbh.bhList.length ; i++ )
    this.removeBehaviour( cbh.bhList[i] );
}

element.prototype.updateComplexBehaviours = function()
{
  for ( var i = 0 ; i < this.complexBehaviours.length ; i++ )
    if ( this.complexBehaviours[i].finishCheck() )
      this.removeComplexBehaviour( this.complexBehaviours[i] );
}

element.prototype.addGoToCBehaviour = function( destPos )
{
  var bhList = [];
  bhList.push( this.makeGoToBehaviour( destPos ) );
  for ( var j = 0 ; j < scene.sceneItems.length ; j++ )
    bhList.push( this.makeDetourBehaviour( scene.sceneItems[j] ) );
  this.addComplexBehaviour( new complexBehaviour( bhList ) , true );
}
...
function gather ( objList, leader, dist, code )	//all elements gather in a closed formation (optionally, around a 'leader') //TODO also try to keep formation
{
  code = code || 3;
  dist = dist || 75;
  objList = objList || elementList;
...
function cancelBehaviours( objList )
{
  ...
  for ( var i = 0 ; i < objList.length ; i++ )
  {
    ...
    if ( objList[i].movementList[0] && !objList[i].movementList[0].pauseBehaviours )		//cancel movements if they were subject to a behaviour
...
element.prototype.increaseBehaviourPriority = function( code, incr )
{
  incr = incr || 1;
  for ( var j = 0 ; j < this.behaviours.length ; j++ )
    if ( this.behaviours[j].code == code )
...
//     alert(scene.sceneItems[0].parts[0].shape.distanceFromFaceTo( elementList[0].pos ));
...
map.prototype.mouseClick = function ( event )	//move selected elements to the map position
{
  ...
  if ( event.button == '0' ) //left button
    for ( var i = 0 ; i < elementList.length ; i++ )
      ...
      {
	elementList[i].addGoToCBehaviour( mapCoords );
