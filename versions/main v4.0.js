...
function doCollisions()
{
  for ( var i = 0 ; i < collisionList.length ; i++ )
    ...
    {
      var colPts = collisionList[i].overlapPoints( collisionList[j] );
      if ( colPts )
      {
	var colPt = collisionList[i].correctOverlap( collisionList[j], colPts[0], colPts[1] );

	if( colPts[0].length >= colPts[1].length )
	  var inObj = collisionList[i];
	else
	  var inObj = collisionList[j];

	collisionList[i].elasticImpact( collisionList[j], inObj.getShape().getNormal( colPt ), colPt );
      }
    }
}
...
rectangle.prototype.ptsInside = function ( item )	//TODO also for a collection (list) of points
{
  var result = [];
...
circle.prototype.ptsInside = function ( item )
{
  var result = [];
  if ( item instanceof vector )
  {
...
ellipse.prototype.ptsInside = function ( item )	// the ellipse is supposed to be vertical (long axis vertical)
{
  var result = [];
  if ( item instanceof vector )
  {
...
element.prototype.overlapPoints = function( obj )
{
    ...
    return result;	//return the collection of intersecting points for each object, or 'undefined'
}
...
rectangle.prototype.overlapDistance = function ( point, vel )	//returns the distance from point to object boundary, in the direction of 'vel'
{
	...
	if ( vel.y > 0 )			//point moved down
	{
	  if ( vel.x < 0 )			//down-left
	  {
	      var x = this.width/2;		//intersection with right wall?
	      var y = a + slopeVel*x;

	      if ( y < -this.height/2 )	//no intersection, then intersection with top wall
	      {
		var y = -this.height/2;
		var x = ( y - a )/slopeVel;
	      }
	  ...
	  {
	      var x = -this.width/2;	//intersection with left wall?
	      var y = a + slopeVel*x;
	...
	result = rectAxes.unproject( point.minus( x, y ).multiply(-1) );	//return an outwards vector
      }
      else		//if 'vel' is undefined or zero
      {
...
circle.prototype.overlapDistance = function ( point, vel )	//returns the distance from point to object boundary, in the direction of 'vel'
{
	...
	var y1 = offset + slopeVel*x1;
	var y2 = offset + slopeVel*x2;

	var p1 = new vector( x1, y1 );
	var p2 = new vector( x2, y2 );

	if( p1.minus( point ).modulus() <= p2.minus( point ).modulus() )
	  var p = p1;
	else
	  var p = p2;
      }
      ...
      {
	var ang = point.angle();
	var p = new vector( this.radius*Math.cos( ang ) , this.radius*Math.sin( ang ) );
      }
      result = circAxes.unproject( point.minus( p ).multiply(-1) );	//return an outwards vector
    }
    return result;
}
...
ellipse.prototype.overlapDistance = function ( point, vel )	//returns the distance from point to object boundary, in the direction of 'vel'
{
	...
	if( p1.minus( point ).modulus() <= p2.minus( point ).modulus() )
      ...
      else		//if 'vel' is undefined or zero//TODO fix
      {
	var ang = point.angle();
	var p = new vector( this.radiusB*Math.cos( ang ) , this.radiusA*Math.sin( ang ) );
      }
      result = ellipseAxes.unproject( point.minus( p ).multiply(-1) );	//return an outwards vector
    }
    return result;
}

element.prototype.correctOverlap = function( obj, thisPts, objPts )
{
    var result = undefined;
    if ( obj instanceof element && this !== obj )
    {
	...
	if ( points )
	{
      ...
      var maxOverlap = new vector( Infinity , Infinity );
      do
      {
	var lastOverlap = maxOverlap.modulus();
	...
	for ( var i = 0 ; i < thisPts.length ; i++ )
	{
	  ...
	  var velP2 =  obj.pointVelocity( thisPts[i] );//TODO use the projected point on the boundary
	  //Total collision velocity
	  var totalVel = obj.velocity().add( velP2 ).minus( this.velocity() ).minus( velP1 );
	...
	for ( var i = 0 ; i < objPts.length ; i++ )
	{
	  ...
	  var velP2 =  obj.pointVelocity( objPts[i] );//TODO use the projected point on the boundary
	...
	var v1 = inObj.velocity();
	var v2 =  outObj.velocity();
	var w1 = inObj.angVelocity();
	var w2 = outObj.angVelocity();

	var colSpd = colVel.modulus();
	var dist   = maxOverlap.modulus();
	if ( colSpd != 0 )
	{
	  var factor =  dist/colSpd;
	  var correct1 =  inObj.velocity().multiply( factor );
	  var correct2 = outObj.velocity().multiply( factor );
	}
	else
	{
	  var factor = 0;
	...
	inObj.setPosition(  inObj.pos.minus( correct1 ) );
	outObj.setPosition( outObj.pos.minus( correct2 ) );

	inObj.rotate ( -w1 * factor );
	outObj.rotate( -w2 * factor );

	//New check, for a new iteration if necessary
	var points = this.overlapPoints( obj );
	if ( points )
	{
	  thisPts = points[0];
	...
	result = colPt.minus( correct2.add( outObj.pointVelocity( colPt, -w2 *factor ) ) );
      }
      while( ( thisPts || objPts ) && maxOverlap.modulus() < lastOverlap )
    }
    return result;		//return the corrected (deepest) point of collision, or 'undefined'
}
