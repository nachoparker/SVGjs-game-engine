//	TODO
//	====
//	* Outer Bounding box to ease checks + quadtrees
//	* selecting box
//	* behaviours:
...
function init()
{
  ...
  collisionList = elementList.concat( scene.sceneItems );
...
function doCollisions( objList )
{
  ...
  for ( var i = 0 ; i < objList.length ; i++ )
      ...
      {
	var colPts = overlapPoints( objList[i] , objList[j] );
	if ( colPts )
	{
	  var rewindData = rewindOnCollision( objList[i], objList[j], colPts[0], colPts[1] );
	  ...
	  for ( var k = 0 ; k < objList.length ; k++ )
	    if ( k != i && k != j && objList[k] instanceof element )
	    {
  ...
  for ( var i = 0 ; i < colPt.length ; i++ )
    for ( var j = i+1 ; j < colPt.length ; j++ )
      if ( colPt[i][j] )
	    elasticImpact( inObj[i], outObj[i], inObj[i].getShape().getNormal( colPt[i][j] ), colPt[i][j] );
}
...
function elasticImpact ( obj1, obj2, dirN, point )	//solve impact according to surfaces, or with a wall with normal 'normalDir'
{
    ...
    {
      obj1.replaceMovement ( new movement( newVel1.direction(), newVel1.modulus(), baseAccel/2 ) );
      obj1.rotation = new rotation( w1f , frictionMoment );
    }

    if ( obj2 instanceof element )
    {
      obj2.replaceMovement ( new movement( newVel2.direction(), newVel2.modulus(), baseAccel/2 ) );
...
polygon.prototype.getNormal = function( point ) 			///get the normal vector in the point of collision
{
    ...
    if ( point instanceof vector )
    {
      var ang = angleCheck2( point.minus( this.pos ).angle2() - this.rotation );	//between [-PI, +PI)
      var alpha = [];
      for ( var i = 0 ; i < this.points.length ; i++ )
  ...
  var returnList = [];
	...
	for ( var k = 0 ; k < polygonList[i].points.length ; k++ )
	  for ( var n = 0 ; n < polygonList[j].points.length ; n++ )
	    ...
	    {
	      if ( !p1 )
		p1 = polygonList[i].pos.add( polygonList[i].points[k] );
	      else
		p2 = polygonList[i].pos.add( polygonList[i].points[k] );
	      n = polygonList[j].points.length;
	    }
	if ( p1 && p2 )
	{
	  //JOIN THE POLYGONS BY THE COMMON POINTS
	  var points1 =  polygonList[i].globalPoints();
	  var points2 =  polygonList[j].globalPoints();
	  while ( !points1[0].equals( p1 ) && !points1[0].equals( p2 ) )	//begin polygon 1 by point p1 or p2
	    points1.push( points1.shift() );
	  while ( !points1[0].equals( p1 ) && !points1[0].equals( p2 ) )	//begin polygon 1 by LAST point p1 or p2
	    points1.push( points1.shift() );

	  var pindex = 0;
	  if ( points1[0].equals( p1 ) )					//truncate it so it begins with one point and ends with the other
	    while ( !points1[ pindex ].equals( p2 ) )
	      pindex++;
	  else
	    while ( !points1[ pindex ].equals( p1 ) )
	      pindex++;
	  points1 = points1.slice( 0, pindex + 1 );

	  if ( points1[0].equals( p1 ) )					//make the same with the second polygon
	  {
	      while ( !points2[0].equals( p2 ) )
		points2.push( points2.shift() );
	      var pindex = 0;
	      while ( !points2[ pindex ].equals( p1 ) )
		pindex++;
	      points2 = points2.slice( 0, pindex + 1 );
	  }
	  else
	  {
	      while ( !points2[0].equals( p1 ) )
		points2.push( points2.shift() );
	      var pindex = 0;
	      while ( !points2[ pindex ].equals( p2 ) )
		pindex++;
	      points2 = points2.slice( 0, pindex + 1 );
	  }
...
function element( id, coordinates )
{
  ...
  this.angle 		= Math.PI/180*parseFloat( this.SVGNode.getAttribute( 'rotation' ) ) || 0;
  this.movementList	= [];
  ...
  if ( readShape == 'circle' )
  {
    this.shape = this.getCircle();
    this.momInertia = this.mass*( this.shape.radius*this.shape.radius )/2;
  }
  else if ( readShape == 'ellipse' )
  {
    this.shape = this.getEllipse();
    this.momInertia = this.mass*( this.shape.radiusA*this.shape.radiusA + this.shape.radiusB*this.shape.radiusB )/4;
  }
  else if ( readShape == 'polygon' )//we assume the polygon is described by an SVG PATH without arcs or bezier curves, all points are relative
				    //(small 'l' command, so we assume it is ommited by default) eg: "d=M 50,50 20,30 -10,-10 z"
  {
    this.shape = new polygon( pointsFromPath( this.group.firstElementChild ) );
    this.height = this.shape.getHeight();
    this.width  = this.shape.getWidth();
    this.pos = new vector( this.shape.pos );
    this.momInertia = Infinity;//    this.momInertia = this.mass/( 6 * SUM Pn+1.modulus()*Pn.modulus*Math.sin(Pn+1.angle(Pn)) )*SUM(xxxx);//TODO
  }
  else
  {
    this.shape = this.getRectangle();
...
function sceneItem( id )			//static solid item that is part of the scenery and cannot be crossed through. Defined by a closed 'path'
{
  if ( !id ) return;
  this.SVGNode = document.getElementById( id );			//the node must be a path
  this.id = id;

  this.shape = new polygon( pointsFromPath( this.SVGNode ) );//new shape 'polygonList'
  this.pos = this.shape.pos;

  if ( this.shape.isConvex() )
  {
    /*this.shape = */
    var aux = this.shape.splitPolygon();
  for ( var i = 0 ; i < aux.length ; i++ )
if (aux[i].isConvex()) alert( "Polygon decomposition failed: " + id );//TODO
  mergePolygons(aux);
  }
}

sceneItem.prototype.getShape = function()
{
  return this.shape;
}

sceneItem.prototype.velocity = function ()
{
  return new vector( 0, 0 );
}

sceneItem.prototype.angVelocity = function ()
{
  return 0;
}

sceneItem.prototype.pointVelocity = function( point, angVel )
{
  return new vector( 0, 0 );
}
