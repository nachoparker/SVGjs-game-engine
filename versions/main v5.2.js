...
//  TODO
//  ====
//  * collisions as events..
//  * animations (normal, sprites??), barritas,
...
function gotest( event )//TODO this is only a test REMOVE
{
  var mapCoords = new vector( event.args[1].pos );
  event.args[0].addGoToCBehaviour( mapCoords, true ); //'true' means its an order
}

addEventListener( 'load', init, false );

//GLOBAL PARAMETERS TODO add more
frameRate = 50;
...
shiftAngle = 20*Math.PI/180;
  ...
  //TEXT
  textDuration = 3000;
  answerStyle = 'fill:yellow;font-size:24px;stroke:black;stroke-width:0.2px';
  questionStyle = 'fill:green;font-size:24px;stroke:black;stroke-width:0.2px';
  txtStyle = 'fill:white;font-size:24px;stroke:black;stroke-width:0.2px';
  lookStyle = 'fill:yellow;font-size:24px;stroke:black;stroke-width:0.2px';
...
function init()
{
  ...
  actionDefs[ 'gotest' ] = gotest;//TODO

  ///GLOBAL ELEMENTS
  useList = collectionToArray( document.getElementsByTagName( 'use' ) );
  frontPanel  = getList( 'type', 'panel', 'svg' )[0];
  scene   = getList( 'type', 'map', 'svg' )[0];

  pausePanel= new text( 'PAUSE' , 'fill:white;font-size:30px' );
  hideElement( pausePanel.SVGNode );

  scene.characterList[0].setSelected( true );
  scene.focus( scene.objectList[0] );
  scene.follow( scene.objectList[0] );
...
function mainLoop()
{
  ...
  doMovements();
  mainEventDispatcher.checkEvents();//TODO
  doCollisions();
  doBehaviourss();
  doAnimations();
  doDrawing();
...
countDownTimer.prototype.resetCount = function( id )
{
  if ( id )
    for ( var i = 0 ; i < this.countDowns.length ; i++ )
      if ( this.countDowns[i].id == id )
...
function event( typeStr, condition )	//event of a certain type. Arbitrary additional arguments can be passed in.
{
  this.type = typeStr;		//the event will be handled by an eventListener of the same type
  this.condition = condition;	//the event will be handled if the 'condition' parameter coincides
  this.args = [];		//extra parameters for the handler to execute the action
  for ( var i = 2 ; i < arguments.length ; i++ )
    this.args.push( arguments[i] );
}

function eventListener( type, action, condition, oneUse )
{
  this.type = type;
  this.action = action;			//receives the event as its only parameter.
  this.condition = condition;		//determines if the event is executed
  this.oneUse = oneUse || false;
}

function eventDispatcher()	//prototype where events are sent to and handlers are published.
{
...
eventDispatcher.prototype.checkEvents = function ()	//Process all events that need to be handled
{
  for ( var i = 0 ; i < this.events.length ; i++ )
  {
    for ( var j = 0 ; j < this.eventListeners.length ; j++ )	//one event can fire many handlers
      if ( this.events[i].type == this.eventListeners[j].type && 
	  this.eventListeners[j].condition && this.eventListeners[j].condition == this.events[i].condition )
      {
...
{
  objList = objList || scene.objectList;
...
{
  objList = objList || scene.beingList;
  for ( var i = 0 ; i < objList.length ; i++ )
    objList[i].animate();
}
...
{
 // doDrawing();//TODO
  colTree = colTree || scene.collisionTree;
  colTree.update();
...
item.prototype = new icon();    //new objects created with 'new' and this function will have this obj as their prototype to look up to
item.prototype.constructor = item;

function item( SVGNode, coordinates )   //an interactive element, without physics
{
  ...
  {
    var act = function(){ action( scene.selectedList, item ); };  //call the action on behalf of the character, over the object
...
item.prototype.setCMenu = function( menuIcons, radius, itemSize ) //layout the menu icons in a circle
{
  ...
  radius  = radius || 1.5 * parseFloat( this.circNode.getAttribute( 'r' ) ) + itemSize  || 80;
...
item.prototype.mousedown = function( event )
{
  ...
  if ( event.button == '2' )    //SHOW MENU
  {
    if ( this.menuGroup.getAttribute( 'display' ) )
    {
      for ( var i = 0 ; i < scene.itemList.length ; i++ )
        scene.itemList[i].hideCMenu();
      this.setCMenu();
      this.placeCMenu();
      this.showCMenu();
    }
...
object.prototype.setSelected = function( selected )
{
  ...
  if ( !selected && this.selected )
  {
      ...
      scene.selectedList.removeElement( this );
      this.selected = false;
  }
  scene.unfollow();
  scene.setMaxZoom();
...
function talk( chList, it )		//Shows the conversation corresponding between acting and acted characters
{
  ...
  if ( it.rootConversNode.length > 1 )
    for ( var j = 0 ; j < it.rootConversNode.length ; j++ )
      for ( var i = 0 ; i < scene.selectedList.length ; i++ )
	//the id is of the form 'actedcharacterIDactingCharacterID'
	if ( it.rootConversNode[j].id.indexOf( it.id + scene.selectedList[i].id ) != -1 )
	{
	  conversation = it.rootConversNode[j];
	  j = it.rootConversNode.length;
	  i = scene.selectedList.length;
	}
...
character.prototype.mousedown = function( event )
{
  ...
  if ( event.button == '0' ) 
  {
    ...
    {
      if ( !( scene.selectedList.length == 1 && scene.selectedList[0] === this ) )  //don't de-select the last one
	this.toggleSelected();
    }
    else
...
// {
//   this.objList = objList || scene.beingList;
...
function keyDown( event )
{
  ...
  if ( event.keyCode == 49 ) //'1'
  {
    ...
    {
      scene.objectList[0].setSelected( true );
//       scene.focus( scene.objectList[0] );
    }
  }
  ...
  {
    selectElements( false );
    if ( scene.objectList[1] )
    {
      scene.objectList[1].setSelected( true );
//       scene.focus( scene.objectList[1] );
    }
  }
  if ( event.keyCode == 51 ) //'3'
  {
    selectElements( false );
    if ( scene.objectList[2] )
    {
      scene.objectList[2].setSelected( true );
//       scene.focus( scene.objectList[2] );
    }
  }
  if ( event.keyCode == 52 ) //'4'
  {
    selectElements( false );
    if ( scene.objectList[3] )
    {
      scene.objectList[3].setSelected( true );
//       scene.focus( scene.objectList[3] );
    }
  }
  if ( event.keyCode == 53 ) //'5'
  {
    selectElements( false );
    if ( scene.objectList[4] )
    {
      scene.objectList[4].setSelected( true );
//       scene.focus( scene.objectList[4] );
    }
  }
  if ( event.keyCode == 54 ) //'6'
  {
    selectElements( false );
    if ( scene.objectList[5] )
    {
      scene.objectList[5].setSelected( true );
//       scene.focus( scene.objectList[5] );
    }
  }
  if ( event.keyCode == 55 ) //'7'
  {
    selectElements( false );
    if ( scene.objectList[6] )
    {
      scene.objectList[6].setSelected( true );
//       scene.focus( scene.objectList[6] );
    }
  }
  if ( event.keyCode == 56 ) //'8'
  {
    selectElements( false );
    if ( scene.objectList[7] )
    {
      scene.objectList[7].setSelected( true );
//       scene.focus( scene.objectList[7] );
    }
  }
  if ( event.keyCode == 57 ) //'9'
  {
    selectElements( false );
    if ( scene.objectList[8] )
    {
      scene.objectList[8].setSelected( true );
//       scene.focus( scene.objectList[8] );
    }
  }
...
//     scene.objectList[0].setPosition( new vector(0,0) ) ;

  //////////////// BEHAVIOURS \\\\\\\\\\\\\\\\\\\\TODO this can be done on an individual basis
  if ( event.keyCode == 96 )    // KP_0
    avoidOthers( scene.selectedList );

  if ( event.keyCode == 97 )    // KP_1
    followInLine( scene.selectedList, scene.objectList[1] );

  if ( event.keyCode == 98 )    // KP_2
    follow( scene.selectedList, scene.objectList[1] );

  if ( event.keyCode == 99 )    // KP_3
    gather( scene.selectedList );

  if ( event.keyCode == 100 )   // KP_4
    gather( scene.selectedList, scene.objectList[1] );

  if ( event.keyCode == 101 )   // KP_5
  {
//     avoidObstacles(scene.selectedList);
     detour( scene.selectedList );
  }

  if ( event.keyCode == 102 )   // KP_6
     keepPosition( scene.selectedList );

  if ( event.keyCode == 110 )   // KP_DEL
    cancelBehaviours( scene.selectedList );
}
...
function map( SVGNode, viewBoxCoords, viewBoxSizeX, viewBoxSizeY ) ///coordinates are CENTER of the view box
{
  ...
  this.objectList = allList.find( object    );
  this.itemList   = allList.find( item    );
  this.beingList  = allList.find( being   );
  this.characterList  = allList.find( character );
  this.selectedList = [];

  this.collisionTree = new quadTreeNode( this.objectList.concat( this.sceneItems )/*.concat( this.itemList )*/, this.mapSizeX, this.mapSizeY  );
  this.collisionTree.buildTree( /*1, 0*/ );
...
map.prototype.viewBoxChange = function( newPoint, newSizeX, newSizeY )           //when there is zoom and pan, not only setViewBox, but also update the front panel info
{
  ...
  {
    this.itemList[i].setSCircle();
    this.itemList[i].placeCMenu();
  }
}
...
map.prototype.insertElement = function( element ) //insert an element into the map and its lists
{
  ...
  if ( element instanceof being )
    this.beingList.push( element );
  if ( element instanceof character )
    this.characterList.push( element );

  this.collisionTree.insertElement( element )
...
{
  this.itemList.removeElement( element );
  this.objectList.removeElement( element );
  this.beingList.removeElement( element );
  this.characterList.removeElement( element );
  this.collisionTree.removeElement( element );
...
map.prototype.mouseClick = function ( event ) //move selected elements to the map position ( with 'shift', put in order queue )
{
  ...
  if ( event.button == '0' ) //left button
    ...
    {
      if ( !event.shiftKey )
        this.selectedList[i].removeAllComplexBehaviours( this.selectedList[i].orders );
      this.selectedList[i].addGoToCBehaviour( mapCoords, true ); //'true' means its an order
    }
}
...
function mouseUp ( event )
{
  ...
  if ( overlapPoints( scene.objectList[i], rect ) )
    list.push( scene.objectList[i] );
...
text.prototype.clean = function()   //remove all lines
{
...
text.prototype.addMouseFunction = function( fn )	//add a function to execute on mousedown event, clean() by default
{
  var that = this;
  fn = fn || function(){ that.clean() };
...
textSequence.prototype.next = function( jump )
{
  jump = jump || 1;
  this.goTo( this.currentIndex + jump );
}

textSequence.prototype.previous = function( jump )
{
  jump = jump || 1;
  this.goTo( this.currentIndex - jump );
}

textSequence.prototype.play = function( intervalms )
{
  intervalms = intervalms || textDuration;
  var that = this;
  this.timer = mainTimer.addCountDown( function(){ that.next() }, intervalms, true );	//'true' means loop
}
...
//function bar( color, pos, length, width )
//{}
//
//bar.prototype.setValue = function(){}  
//
//bar.prototype.setPercentage = function(){}
